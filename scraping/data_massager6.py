...
def massage(file1):
    ...
    for row in reader1:
        ...
        rating.append(row[6])
        image.append(row[7])
        imageMap.append(row[8])
        imageMapXL.append(row[9])
        updateDate.append(row[10])
        snowDepth.append(row[11])
        status.append(row[12])
        city.append(row[13])
        state.append(row[14])
    ...
    writeResort(resortCSV, Resort, ticketPrice, difficulty, startSeason, endSeason, image, imageMap, imageMapXL, rating, city, state)
...
#    maxSnow = 500.0;
    ...
    startCities = ['Chicago', 'New York', 'Washington D.C.', 'Boston', 'Los Angeles', 'Seattle', 'Atlanta', 'Miami', 'Dallas', 'San Francisco']
    #date = '2018-03-23'
    for computedDate in dateExtent:
